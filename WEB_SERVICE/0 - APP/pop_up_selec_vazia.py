...
def def_pop_up_vazio(root):
    ...
    frame_delete.title("Alerta")
    frame_delete.geometry("450x250")
    ...
    texto = ctk.CTkLabel(frame_delete, pady=40, text="Selecione um valor para que possa ser excluído", width=200, height=25, font=('arial bold', 18))
    texto.pack()

    btn_ok = ctk.CTkButton(frame_delete, text='OK',command=lambda: [fechar_janela(frame_delete, root)])
    btn_ok.place(x=155, y=175)
